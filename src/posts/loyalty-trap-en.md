---
title: "The Loyalty Trap: A Verification Game of Xi's Military Purge"
date: "2026-01-28"
tags: ["Military Purge", "Dictator's Dilemma","Authoritarianism"]
draft: false
---
...
```mermaid
flowchart TD
  A["Stage One: The Loyalty Trap<br/>(Multitask Principal-Agent Model)"] --> B["Exogenous Shock<br/>(ω Rises)<br/>• 2027 Timeline Near<br/>• Russian Failure<br/>• US Intel Disclosure"]
  B --> C["Stage Two: Verification Game<br/>(Equilibrium Switching Model)"]
  A --> A1["Peacetime → Observability Difference → Effort Distortion<br/>→ Performative Loyalty Equilibrium<br/>→ Corruption Accumulates (Years to Decades)"]
  C --> C1["μ(S)·ω·L > K → Equilibrium Switch → From Trust to Audit<br/>→ Massive Purge"]
```
...
flowchart TD
  ...
  General --> Work[Work]
  General --> Shirk[Shirk]
  Work --> R1["High readiness"]
  Shirk --> R0["Low readiness"]
  R1 --> S1["Signal: Good"]
  R0 --> S0["Signal: Good or Bad"]
  S1 --> Dictator1{Dictator}
  S0 --> Dictator2{Dictator}
  Dictator1 --> Retain1["Retain (Game End)"]
  Dictator1 --> Audit1["Audit (R=1)"]
  Dictator2 --> Retain2["Retain (Higher war-loss risk)"]
  Dictator2 --> Audit2["Audit (Purge R=0)"]
```
...
```mermaid
flowchart LR
  subgraph Trust["Trust Zone (μ·ω·L < K)"]
    T1["Peacetime Norm"]
    T2["Tolerate Corruption"]
    T3["PLA before 2023"]
  end
  subgraph Audit["Audit Zone (μ·ω·L > K)"]
    A1["War Risk Rises"]
    A2["Verify at all costs"]
    A3["Rocket Force after 2023"]
  end
  Trust --> Audit
  Note["Critical Line: K = μ·ω·L_lose<br/>2023 Switch Point (Russian Failure / Intel / 2027 Timeline)"]
  Trust --- Note
  Audit --- Note
  KHigh["K High"] --> KLow["K Low"]
  OmegaLow["ω Low"] --> OmegaHigh["ω High"]
```
